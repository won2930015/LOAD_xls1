...
def writexls(xlsdata,path):
    ...
    val1=0
    val2=0
    for i,item_ in enumerate(Labels[:]):
        if i>0 :i+=3
        sheet1.write(Y, i, item_)#填充标题
...
##        sheet1.write(Y, 0, fstr[-2]+'.'+fstr[-2])#日期
        sheet1.write(Y, 0, "%d.%d"%(int(row[0].split('-')[-2]),int(row[0].split('-')[-1])))#日期
##        sheet1.write(Y, 0, row[0])#日期
        ...
        sheet1.write(Y, 7, row[3].split('/')[0]) if len(row[3])>7 else sheet1.write(Y, 7, row[3])#柜号
        for rowd in row[-1]:
            val1+=rowd[1]
            val2+=rowd[2]
...
def main():
    load_path='G:\\test222\\LOAD_xls\\test.xls'
    out_path='G:\\test222\\LOAD_xls\\test_out.xls'
    writexls(loadlxs(load_path),out_path)
